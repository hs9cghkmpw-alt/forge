        ...
        **AIに委ねる範囲を意図的に狭くしている**: ここでAIが決めるのは
        ...
        return Prompt(
            ...
            system=(
                ...
                "   - 「やることを忘れる」→ やることだけで足りる。\n"
                "   - 「釣果を細かく記録したい」→ 魚種・サイズ・場所・日付が要る。\n"
                "   ユーザーが**言っていない情報を、念のため足さないこと**。\n"
                "   「あると便利かも」で増やした項目は、毎回の入力を重くするだけである。\n"
                ...
                "空のデータが増えていくのを防ぐため)。\n"
                "8. type=numberの項目には、その数値が**どういう量か**を"
                "measureで答えること。**足せる量かどうかは型では決まらない**。\n"
                "   - additive    : 足し合わせに意味がある。金額・数量・歩数・距離。\n"
                "                   「全部でいくら/いくつ」が答えになる。\n"
                "   - averageable : 平均に意味がある。評価・点数・満足度。\n"
                "                   **合計は無意味**(『評価の合計が42』は何も言っていない)。\n"
                "   - level       : ある時点の水準。体温・体重・残高・在庫水準。\n"
                "                   知りたいのは普通『今いくつか』である。\n"
                "   - extremum    : 最大/最小に意味がある。魚のサイズ・自己ベスト。\n"
                "   - identifier  : 集計しない数値。年号・部屋番号・ページ番号。\n"
                "   - unknown     : 上のどれとも言い切れない場合。\n"
                "   **迷ったらunknownにすること。** 分からないのにadditiveと答えると、"
                "足しても意味の無い数値が画面で一番大きく表示される。\n"
                "9. visual_styleは、その記録が持つ雰囲気に合うものを"
