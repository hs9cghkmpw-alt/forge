...
class TestForgeLanguageCompilerRecordSchema(unittest.TestCase):
    ...
    def test_version_is_1_12(self) -> None:
    ...
    def test_widget_composition_reflects_sprint1_section_headers(self) -> None:
        ...
        # v1.11(FORGE-R1、TD69)で先頭に`metric_view`(Hero KPI)が入った。
        # **順序に意味がある** — 主KPIは一覧より前でなければ「一覧の
        # おまけの合計」になってしまう(`_build_hero_metric_widget`参照)。
        self.assertEqual(
            [c.type for c in list_tab.children],
            ["metric_view", "record_list_view", "bar_chart"],
        )
...
class TestForgeLanguageCompilerV1_7WidgetVocabularyExpansion(unittest.TestCase):
    ...
    def test_document_version_is_at_least_1_7(self) -> None:
        ...
        self.assertIn(document.version, {"1.7", "1.8", "1.9", "1.10", "1.11", "1.12"})
...
class TestForgeLanguageCompilerV1_8WidgetVocabularyExpansion(unittest.TestCase):
    ...
    def test_document_version_is_1_12(self) -> None:
