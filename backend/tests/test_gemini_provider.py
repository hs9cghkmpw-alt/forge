...
ここでは`httpx.MockTransport`で実際のネットワーク呼び出しをすべて
置き換えている。つまり、CI等キー無しの環境でも毎回決定的に実行できる
Unit Testとして、「リクエストの組み立て方」「レスポンスの解析方法」
「エラー時にRuntimeErrorへ変換する経路」を検証するためのものであり、
実際のネットワーク呼び出しはしない。

**2026-08-10追記**: 実際のGemini APIへの接続自体は、このテストファイルでは
なく、CEOが共有した実際のAPIキーを使ってこのセッション内で別途(uvicorn
起動+`curl`)確認済み(`TECH_DEBT.md` TD15参照)。ここのUnit Testは
「壊れていないことを毎回安く確認する」ためのものであり、実API確認の
代わりではない(役割が違うだけで、どちらも必要)。
