...
`/converse`・`/generate`のエラー系テスト(`test_converse_and_update_http.py`
等)がここを通っており、GitHub Actionsがpushごとに実行している。

「fastapi不在で一度も実行できていない」という記述は古かったので訂正した
(013 §8)。ただし**下記のJSON不正 vs スキーマ不正の判定**については、
別途「実際に確認したか」を読むこと——ファイルが動くことと、個々の
判定が正しいことは別である。
