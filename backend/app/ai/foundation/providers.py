...
#: 話題が分からなかったときの例示。**空にする**（Quality Gate v2、2026-08-26）。
#:
#: 以前は `("最初の項目", "2つめの項目")` を入れていた。実描画したところ、
#: 子ども向け・写真・ゲーム・データ分析・学習アプリのどれもキーワードに
#: 当たらず、**全部この文字列が並んだ画面**になっていた。
#:
#: `最初の項目` は道具の中身ではなく**Forgeの内部語**である。利用者から
#: 見れば、開いた瞬間に知らないデータが2件入っている状態になる。
#:
#: 例示は「分かるとき」だけ出す。分からないなら**空状態を見せる**方が
#: 正しい——`checklist` は `empty_state_text` を持っており、そちらは
#: 「まだありません。追加してください」と、次にすることを言える。
#:
#: これは同じ穴の2度目である（#29「mockの品質: 内部識別子を出さない」で
#: 一度直したが、**fallback経路だけ残っていた**）。
_DEFAULT_EXAMPLES: tuple[str, ...] = ()
