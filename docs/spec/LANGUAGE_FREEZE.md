...
## 追記(2026-08-11、FORGE-AI-QUALITY-001、CEO承認によりFreeze運用を解除)

**本ドキュメントの「Widgetを追加しない」という運用は、実際には一度も
正式なFreeze宣言として発効していなかった。** 1章の表が示す通り、
2章のFreeze条件のうち「実Runtimeでの`flutter analyze`確認」「全Widget
種別の実描画確認」は、このセッション時点でもなお未充足のまま
(Flutter/Dart SDKがサンドボックスに無いという既知の環境制約による)。
つまり、これまでWidget追加を避けてきたのは「正式なルールに違反する
から」ではなく、「まだ検証していない未確定の状態を、慎重側に倒して
扱っていた」というだけだった。

この事実を、CEOから「いまの生成できるアプリはテキストとチェック
ボックスぐらいの機能しか持たせられないってこと?これはツールを
それぐらいしかもってないから?ゴールはわかってる?」という直接的な
問いを受けた際に確認・報告した。あわせて、この制約による具体的な
実害——`household_budget`の既存の例文
(`frontend/lib/features/app_generation/presentation/widgets/
example_picker_sheet.dart`の「収入や支出を記録して、月ごとの収支を
グラフで見たい」)が、Widget語彙にグラフ相当のものが存在しないため
文字通り実現不可能な約束になっていたこと——を報告した。

CEOから「凍結宣言をすべて解除します。ひきつづきすすめて。」という
明示的な承認を得て、`choice_field`(ドロップダウン)・`bar_chart`
(棒グラフ)の2 Widgetを追加した(v1.6、`TECH_DEBT.md` TD34参照)。
これにより、7.1節が「将来Widgetを追加する際」として保留していた
`string_list`表示用Widgetの検討も含め、**本ドキュメントの「今後数年
変更しない」という前提そのものが撤回された**。Backward Compatibility
の保証内容(4章)自体は今後も有効(v1.0〜v1.5で合格した文書はv1.6でも
引き続き合格する、実際に回帰テストで確認済み)だが、「Widgetを
増やさない」という運用上の縛りはもう無い。

今後Widgetを追加する場合も、4章(Backward Compatibility)・6章
(Deprecation)の規約自体は引き続き守ること——撤回されたのは
「増やさない」という運用判断のみで、「増やす際にどう互換性を保つか」
の規約は今回変更していない。
