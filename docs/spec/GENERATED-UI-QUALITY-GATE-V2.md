...
## 付則: 操作して撮る（R5 で追加、2026-08-27）

R4 までは**開いた直後の1画面**しか撮れていなかった。R4 の manifest は
「撮れていないだけ」と正しく書いたが、**書いただけでは次も同じ**である。

R5 から `scripts/capture_quality_gate_r5.py` が Playwright で
**実際にタブを押してから**撮る。

```
app を開く → flutter-view を待つ → タブを座標で押す
  → 落ち着くまで待つ → 押す前と絵が変わったことを確かめる
  → 撮る → **画像を開いて人が見る**
```

### 押せなかった操作は失敗である

1件でも押せなければ script は **exit 1** を返す。
**撮れた枚数だけを見て PASS にしない。**

### 「押したつもり」の穴（R5 で実際に踏んだ3つ）

| # | 穴 | 対処 |
|---|---|---|
| 1 | 本文は `Center` + `maxWidth` なので、**画面幅を割った座標は広い画面で余白へ落ちる** | 座標を `forge_renderer.dart` の値から読む。写経しない |
| 2 | `InkWell` の hover は**タブが変わらなくても**絵を変える | 撮る前にカーソルをタブ列の外へ動かす |
| 3 | もともと選ばれているタブは**押しても変わらない** | 別のタブを経由してから戻る。往復の両方で変化を要求する |

穴 2 のせいで、mobile の「一覧タブ」は切り替わっていないのに成功扱いに
なっていた。**差分が出たことは、操作できたことの証明ではない。**

### 撮る対象は「その時の build」でなければならない

R5 の1回目は**前日の build** に対して走っていた（fixture は当日
再生成済み）。**古い build の絵は Evidence ではない。**

撮る前に、fixture を書き出した時刻より新しい build であることを確かめる。

### 同じ絵が複数あることは、必ずしも欠陥ではない

Semantic Layout Composition により、要約が先頭タブの app では
「一覧 state」＝「開いた直後の画面」になる。この一致は**正しい**。

見るべきは **app をまたいだ一致**である（R3 で analytics / game / study
が全 viewport でバイト単位一致していたのが TD87 の姿）。
