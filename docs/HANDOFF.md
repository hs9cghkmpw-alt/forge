...
## 独立レビューの指摘を直しました（2026-09-04 / 001A）

**指摘は正しく、私の作りが甘かったです。**

### 一番大きいもの: 偽の印で通せた

Registry は「Gate を通った印」が**空でないか**しか見ていませんでした。
なので `promotion_decision_digest="fake"` と書くだけで、Gate を通さずに
正式採用できました。**修正前に実際に再現しました。**

私は「`promoted()` だけが印を埋める」とコメントに書いていましたが、
**コメントは保証ではありません。** Python では横から書き換えられます。

**直し方**: 印を信じるのをやめ、**判定に使った入力一式**を持たせて、
受け取る側が**もう一度 Gate を走らせる**ようにしました。

> 以前: 「通ったよ」という印を信じる
> いま: 「通ったと言うなら、その入力でもう一度やってみせろ」

偽造するには「本当に Gate を満たす入力」を作るしかなく、それは偽造では
ありません。

### 2 つめ: 検証後の書き換えが本番で防げていなかった

Manifest の digest は、**両方空なら検査されない**作りでした。しかも本番は
それを渡していませんでした。つまり「防げている」と書いた試験は人工的な
もので、本番の証明になっていませんでした。本番へ配線し、空なら拒否に
しました。

### 3 つめ: 説明と実装が食い違っていた

依存の素性が不明（UNKNOWN）なときの既定が、説明では「拒否」、実装では
「許可」でした。**説明の側を正として、既定を拒否へ揃えました。**
許可する経路は明示的に選び、選んだ事実が記録に残ります。

### 自分の新しい試験が 4 件、置物でした

破壊試験を全数回したところ、**書いたばかりの試験のうち 4 件が
「守っているつもりで何も守っていない」**と判明しました。直しました。
最終的に 36 件すべてが「壊すと落ちる」状態です。

### CEO へのお願い（Windows 実機、1 本だけ）

Windows の Sandbox 自体は**触っていません**。ただし正式採用の本番配線を
変えたので、その部分だけ実機で確かめてください。**コマンドは 1 本です。**
...
最後に `RESULT: ALL PASS` か `RESULT: FAIL (...)` が 1 行出ます。
**その行だけ貼ってください。** TD110 の全再実行は不要です。

---

## Promotion 安全 Gate を入れました（2026-09-04 / FORGE-PROMOTION-HARD-GATE-001）

**AI が書いたコードを Forge 本体へ正式採用してよいかを、1 箇所でまとめて
判断するようにしました。**

これまでは「ビルドが通った」「テストが通った」で採用できました。いまは
次のどれか 1 つでも欠ければ採用しません。

- どんな権限を使うのかの申告（**申告が無いなら「安全」ではなく「拒否」**）
- 危ないことをしていないか（ネット接続・秘密探し・外部コマンド実行など）
- どの檻で動かして確かめたか（**「檻なし」で確かめた結果は認めない**）
...
このリポジトリは「作ったが本番から呼ばれない」を 10 回以上繰り返しています。
なので Gate は呼ぶ側の善意に頼らない形にしました。

| 迂回の試み | 何が止めるか |
|---|---|
| Gate を通さず昇格 | 引数が必須なので `TypeError` で止まる |
| 拒否された決定で昇格 | `PromotionDenied` |
| 横から PROMOTED を名乗る | Registry が「Gate 通過の証」が無いことを見て拒否 |
| 保存して読み直して誤魔化す | Store が同じく拒否 |

### 配線したら既存の穴が 5 件見えました

一番大きいのは、**Promotion が「檻あり」と「檻なし」を区別できていなかった**
ことです。`sandbox_backend` が Promotion まで届いていませんでした。
`os_isolated` という判定は**定義だけあってどこからも呼ばれていません**でした。

また、判定が 2 箇所にあって**中身が違い**、`linux-namespace-fake` のような
偽名を「檻あり」と数えてしまう状態でした。自分で攻撃して見つけました。

### 壊して確かめました

**Critical Guard 26 件を 1 つずつ壊し、全件でテストが落ちることを確認**
しました（26/26）。置物は 0 件です。

以前は手で壊していたので「今回壊した分だけ」しか分かりませんでした。
いまは一覧を持ち、**Gate を足して破壊試験を足し忘れると落ちます**。
この歯止めは導入直後に実際に 1 件の穴を見つけました。

### CI で 1 度落ちました（隠さず書きます）

Dart の相対 import（`import 'capability_impl.dart';`）を「外部依存」と
誤検出し、**実 Dart を要するテスト 4 件が CI でだけ落ちました**。
ローカルは Dart が無いので skip され、気付けませんでした。

同じ形を **Dart 無しで**固定する回帰試験を足したので、以後は CI を待たず
ローカルで落ちます。壊して両方向の検出も確認しました。

### まだ言えないこと

- **「生成コードは安全」とは言えません。** 静的検査は書かれたものしか見ません
- **危険検体は私が書いたものです。** 未知の書き方への見逃し率は未測定
- **実際の獲得試行（Episode）が 0 件**です。したがって Status は
  `IMPLEMENTED` までで、`VERIFIED` へは上げていません（TD120）
- **重大脆弱 Dependency 0 件とは言えません。** 脆弱性情報を持っていません（TD113）
- **macOS は未実装**です（TD117）
- `99_PROVEN` / `HARD_GATE_PROVEN` は **0 件のまま**です

### Windows について

**弱めていません。** `windows_appcontainer.py` は 1 行も触っていません。
`windows-appcontainer+job` は OS 隔離として通ることを試験で固定しました。
Windows 実機の再試験は**不要**です（Windows 固有 backend を変更していないため）。

全文は `docs/reports/FORGE-PROMOTION-HARD-GATE-001-report.md`。
