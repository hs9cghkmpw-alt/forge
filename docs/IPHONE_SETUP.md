...
## 2026-08-11追記: Windows環境の場合

CEOの環境がWindowsであることを確認しました。**この章より下(1〜8章)は
Macが前提の手順**なので、Windowsのままでは実施できません。Windows環境で
iPhoneに触れるための現実的な選択肢は、以下の2つです。

### A. まずはWebブラウザで試す(今すぐできる、追加費用なし・おすすめ)

Macも審査も要りません。`GETTING_STARTED.md`の手順どおりパソコン
(Windows)でBackend・Frontendを起動し、iPhoneのSafariから
`http://(パソコンのIPアドレス):8080`のように開くだけです
(パソコンとiPhoneを同じWi-Fiに繋ぐ必要があります)。ホーム画面に
追加すれば、アイコンっぽく起動できます。**「本物のiPhoneアプリ」では
ありませんが、実際に触って動作確認するには十分**です。

### B. 本物のiPhoneアプリが欲しい場合(Macを使わずにビルドするサービスを使う)

自分でMacを買わなくても、**クラウド上のMacでビルドしてくれるサービス**
(例: Codemagic、Bitrise、Xcode Cloud等)を使えば、Windowsのまま
iPhone向けアプリをビルドできます。一般的な流れ:

1. サービスにサインアップ(Codemagicは無料枠あり)
2. このGitHubリポジトリ(`hs9cghkmpw-alt/forge`)を接続する
3. サービス側の設定で「Flutter」「iOS」ビルドを選ぶ
4. ビルドが終わると、Apple社の**TestFlight**というアプリ経由で
   自分のiPhoneへインストールできる(App Store審査は不要、招待制の
   配布)

**正直な申告**: このセッションではCEOのアカウント作成・外部サービスへの
サインアップが必要なため、Claude側で実際に試すことはできていません。
どのサービスが今の要件(無料枠・使いやすさ)に合うかは、実際に2〜3個
比較してみることをおすすめします。もし「Codemagicで試したい」等、
方向性が決まれば、リポジトリ側に必要な設定ファイル(`codemagic.yaml`等)
を用意する作業はお手伝いできます。

いずれにせよ、TestFlight配布・App Store配布いずれの場合も、Apple
Developer Program(年額)への登録がいずれ必要になります(TestFlightの
「内部テスト」は開発者アカウントのみで一定人数まで無料枠がありますが、
サービスによって条件が異なります)。

---

## 0. 今の状態(以下、Mac環境向けの手順)
