...
- 016A の A / B、017A の A1 / A2 / A3 / C / D まで実装・push済み
- テスト: backend 1407 / forge_ai 521
- **判断項目 F は決着した**（仮名ID = 二層化）。未決は A〜E
...
| ~~F~~ | ~~端末を跨いで辿れる仮名IDを持つか~~ | ✅ **決着（2026-08-24、017A §11）— ①二層化を採用** |

---

## 0-F. ✅ 決着 — 仮名IDは「二層化」を採用（2026-08-24）

CEOの判断（FORGE-017A §11）により**①二層化**で確定した。**この項目は
閉じている。** 以下は決定の内容と、決定に伴う制約である。
...
「セッションを跨いで個人を辿れる識別子を持たない」という既存の約束は、
ローカル側については**そのまま維持される**。

### 併せて決まった制約（重要）

> **client-generated install ID だけを Poisoning 防止の Truth にしない。**

端末側で作った IDは端末側で作り直せる。それだけを「1人あたりの投稿数」
の根拠にすると、**作り直すだけで制限を外せる**。

将来は **server-issued contributor token** または
**authenticated pseudonymous subject** を使い、
rotation / revocation / deletion に対応する。

**仮名IDを持っただけで Sybil 対策が済んだとは言わない。**

### 実装状況

⬜ **未実装。** 決まったのは方針であり、Learning Event を作って送る経路
そのものが commit E である。
