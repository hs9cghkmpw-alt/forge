...
* **フロントエンドの主要導入体験の変更**(Home画面の「何のアプリを
  作りますか」的な文言を「最近、困ってることある?」的な文言へ変更し、
  Inspiration Cardsの遷移先を「即生成」から「会話開始」へ変える判断)。
  これは指示書28章「製品思想そのものを変更する判断」に該当する、
  ユーザーが最初に触れる体験の変更であり、Prototypeの主要体験
  (Home→生成→Tool)を置き換える意思決定にあたるため、実装はせず
  設計(design doc C章)のみに留めた。
