...
`Need → keyword → Domain → Template → checklist` の圧縮を、
`Need → Semantic Role → Capability Plan → IR` へ置き換えた。
Round 4 の実描画 **32枚に重複が1枚も無い**（第3回は analytics / game /
study が全 viewport でバイト単位一致していた）。

専用 Template は1つも作っていない。`PlanShape` は5値で、Shape 名に
need 由来の語が入らないことをテストが固定している。

### ~~TD89. Domain 判定が外れる~~ → **解消**

「子ども」は actor、「旅行」は context として扱い、**構造を決める役から
外した**。実描画で体重・身長も持ち物リストも消えた。

キーワード表は残っている。変えたのは**表の権限**である。

### TD90. 作れないと分かっているのに、利用者へ言わない（2026-08-26、新規）

**Round 4 の Golden Gate が FAIL である最大の理由。**

「植物を育てながら音を組み合わせるゲーム」は、**植物と音を記録する CRUD**
になる。`CapabilityPlan.unsupported` は `simulate.loop` と `media.compose`
を**正しく名指ししている**——Forge は作れないと知っている。

しかし**その事実が利用者に一切届かない**。画面にも会話にも出ない。

これは「無いものを checklist で代用して黙る」の**最後の残り**である。
Plan まで来た `unsupported` / `partial` を、生成物か会話に載せる必要がある。

直し方の候補: (a) 会話で「その部分は作れない」と言う（Capability Layer は
既に在る）/ (b) 生成物に「この機能は未対応」の表示を持たせる。

### TD91. record_log 系アプリの入口が似ている（2026-08-26、新規）

photo / map / game / analytics / study はどれも **3タブ CRUD + フォーム**
で始まる。Shape（record_log / +group_compare / +trend）の違いは一覧タブ側に
出るので、**入口だけ見ると区別が付かない**。

TD87 のような「同一」ではない（32枚に重複は無い）。「第一印象が同じ」で
ある。Shape ごとに入口の構成を変える余地がある。

### TD92. 撮影ハーネスが第1タブしか写せない（2026-08-26、新規）

`analytics.json` には `bar_chart` と `metric_view(aggregate=sum)` が
**実在する**が、一覧タブにあるので静止画に写らない。

**「撮れていない」と「無い」は違う。** 現状の Quality Gate は前者を
後者と読み違えうる。タブを操作して撮る、または撮影対象の初期タブを
指定できるようにする。

### ~~model_digest を「数える条件」にしていた~~ → 分離（020A1）

`WeightIdentity` を新設し、digest は **Level 0.5 の条件**へ移した。
1つの欄に「重みの同一性」と「fixture 除け」を兼務させていたので、
digest を返さない本物が永久に到達できず、digest を返す偽サーバは
通っていた。**緩めたことを隠さない。** 代わりに `domain_resolution` と
`observed_tasks` の2検査を足した。
...
### ~~TD86. アプリ名が生の要求文のまま~~ → **第3回で解消**（2026-08-26）

`forge_ai/core/naming.py` を新設し、compile の2経路の**両方**を通した。
候補を `is_name_like()` へ通し、通ったものだけ名前にする。通らなければ
Domain の日本語名、それも無ければ `新しいアプリ`——**分からなかったと
認める**。

願望文から名詞句を削る処理は1つも足していない。**削らずに落とす。**
配線破壊試験6件すべてで対応テストが落ちる（置物なし）。

### ~~TD89. Domain 判定が外れる~~（**解消。上記参照**）

8件中2件が別分野と判定された。

| 要求 | 実際に作られたもの |
|---|---|
| 子どもが朝の支度をひとつずつチェックできるようにしたい | `child_growth` →「こどもの成長」＋体重測定・身長測定 |
| 旅行の写真を日付ごとに残してメモを付けたい | `travel` →「旅行」＋充電器・着替え・歯ブラシ |

**第2回まではこれが見えていなかった。** 要求文がタイトルに出ていたので、
画面が要求とずれていても**タイトルだけは正しく見えていた**。名前を
名前にしたことで、ずれが画面へ出るようになった。

これは TD86 の修正が作った不具合ではない。**元からあった誤判定が
見えるようになった**だけである。隠れている不具合より見えている方がよい。

直し方: Domain 判定は「子ども」「旅行」といったキーワードの substring
一致である（`lexicon.py`）。**何についての道具かではなく、どの語が
出てきたかで決めている。** TD87（Capability 分解）と同じ根。

### ~~TD87. 8アプリが3種類の画面にしかならない~~（**解消。上記参照**）
...
**CEO判断済み（2026-09-02）**: releaseで無言消失を維持しない。未対応または
...
## TD96. 要求理解が語の一致に頼っていて、言い回しを取りこぼす（2026-08-31）

ランダム自由文E2Eを入れた瞬間に出てきた。固定文では見えなかった。

200通りの自由文で測った読み取り率:

| | 読み取れた |
|---|---|
| 既存能力だけで作れる要求 | 200 / 200 |
| 月ごとに見たい要求 | 117 / 200 |

同日中に2つ直した（「月ごと/月単位/月別」をCatalogの検出語へ追加、
「〜た日/〜の日」を日本語の形で日付として受ける）。**残っているもの**:

* 「どの月に何回あったか分かるようにしたい」→ 月表示の要求だと読めない
* 「会う日」「行く日」のような動詞終止形＋日 → 日付だと読めない

語を1つずつ足す方向で埋めない。分野の数だけ増えて追いつかない。
意味側で扱う設計が要る。E2Eは取りこぼした文を
`comprehension_misses` としてログへ残す（無かったことにしない）。

## TD97. 獲得した能力の再利用がprocessローカル（2026-08-31）

`PROMOTED_CAPABILITIES`はprocess内のoverlayである。同じprocessの中で
再生成が走らないことは実測で確かめた（方式B、生成0回）。しかし
**Forgeを再起動したあとも獲得済みのままか**は別の話であり、確かめていない。

`frontend/lib/json_ui/acquired/`のinstall自体はディスクに残るので、
Dart側は再起動後も描ける。ずれるのはPython側のRegistryである。
「1度作った能力は次回から速い」を製品として名乗るには、この永続化が要る。


## TD98. BUILD段の生成時間を実機で測っていない（2026-09-01）

**計測器は入れた（同日）。** `stage_timing.py` で段ごとの実測を本番経路で
取り、`/converse` の応答へ `timings` として返す。実機は
`python3 scripts/measure_real_device_converse.py` の1コマンドで測れる。
配線破壊試験7件すべて検出。

**しかし実機の数字はまだ無い。** このコンテナにOllamaが無いため、
確かめたのは`provider=mock`での動作だけである。実機で測るまでPASSにしない。

会話の判定は速い道で 73.54秒 → 0.09ミリ秒になった。しかしBUILDへ進んだ後、
`PromptPipeline`が小型ローカルモデルでentity synthesisと検証を回す。
**そこが何秒かかるかを実機で測っていない。**

`receiveTimeout`を60秒へ広げたが、それで足りるかも確かめていない。
...
73.54秒でDioが先に切れる」だった。速い道でその壁は越えるはずだが、
**実機Chromeで見ていないのでPASSと書かない。**

確認すべき順:
1. `/converse`へ実機POST（応答時間とstatus=build）
2. 生成まで含めた実時間
3. Chromeから同じ文で画面到達
4. 「家族で予定を管理したい」でちゃんと聞き返すか（雑になっていないこと）
...
## TD102. 73.54秒→0.09msを全体の高速化と読めてしまう書き方をした（2026-09-01）→ **解消**

あの数字は最初の「ASK/BUILD判定」だけのものであり、Forge全体の処理時間では
ない。倍率（約81万倍）で書いたため、全体が速くなったように読める報告に
なっていた。CEO指摘により訂正。以後、倍率では言わず「判定が0.09ミリ秒」と
だけ言う。BUILD段は未計測（TD98）。

## TD103. `Real Local Model runs = 0` の定義が曖昧だった（2026-09-01）→ **解消**

「実Modelがcapabilityの実装を書いた証拠が無い」と書いていたが、これでは
「Local Modelを動かしていない」と読めてしまう。実際には実モデルによる
**会話経路は実機PASS済み**である。
