...
CI で 1 度落ちた（run 33864738810）。Dart の相対 import を外部依存と誤検出し、
実 Dart を要する 4 件が **CI でだけ**落ちた——ローカルは Dart が無くて skip される。
Dart 無しで同じ形を固定する回帰試験を足し、配線破壊で両方向の検出を確認した。
...
- ホーム画面の `_ProviderToggle`（`Gemini` ⇔ `Mock` のタップ切り替え）を削除。
  Provider 名を通常利用者へ見せないだけでなく、**利用者に AI 経路を選ばせない**
  （Constitution §4・§9）。片方の選択肢が疑似データだった点でも方針違反だった。
- `frontend/lib/shared_widgets/ai_mode.dart` を追加。`AIモード` /
  `AIが考えています…` / `AIモードを準備しています…` / `お試しモード` /
  `AIに接続できません` の5状態を持ち、**表示が実状態と一致する**ことを型で守る。
  判定は楽観側へ倒さない順序に固定（unreachable > mock build > backend simulated
  > busy > ready）。`AiModeIndicator` は押せない表示専用部品。
- 開発者向け Provider 指定は `--dart-define=FORGE_DEVELOPER_MODE=true` の
  ビルドにだけ存在する（`developer_mode.dart` / `DeveloperProviderOverride`）。
  既定ビルドには存在しない。
- Backend: `provider_independent_messages.py` を追加し、
  `exception_handlers.py`（利用者向け文言が通る唯一の場所）で Provider の
  身元を含む文言を Provider 非依存文言へ差し替える。`_no_provider_message`
  から「別の AI Provider を設定してください」を除去。**内部の `exc.message`、
  ログ、Evidence、`providerUsed` は実 Provider 名を持ち続ける。**
- **実描画で見つけた嘘 1**: `MockConversationRepository` が `simulated: true`
  を付けておらず、疑似データが `Generated by Forge` と名乗っていた。
  Repository を直したうえで `isSimulatedOutput()` がビルド自体の事実を必ず
  OR する形にした（呼び出し側の付け忘れで本物に見せない）。
- **実描画で見つけた嘘 2（TD92 解消）**: `ForgeFallbackWidget` が Release で
  `SizedBox.shrink()` を返し、描けなかった部分が黙って消えていた。Release でも
  必ず「この部分はまだ表示できません」を描く。内部語彙は出さない。
- テスト +27（frontend 562→589）、backend +12（2023→2035）。
  配線破壊試験 5 種すべてで対応テストが FAIL することを確認。
- Visual Evidence: `docs/evidence/visual/provider-independent-ui-20260902/`
  （Chromium 実描画、1440 / 820 / 390 / 320 の4 viewport ＋ 疑似ビルド2枚）。
- **事故報告**: 実描画の準備中、作業ホストの `backend/.env` に実 API キーが
  入っていることに気付かず backend を起動し、`/converse` を 2 回送信した。
  実 Gemini API が呼ばれた可能性が高い。直ちに停止。詳細は HANDOFF 冒頭。
- 新規 TD104: 呼んでいない Provider の名前を Evidence が名乗る（fast path で
  `provider: "gemini"` を観測）。表示はしていないが Evidence の正確さの問題。

# 2026-09-02 — 全方針監査とUniversal Quality Invariant

- CEOの過去方針、添付構想、上位正本文書、0円・全項目99%戦略を厳格照合し、
  PC/GPU/RAM/OS/端末/無料・有料/Local・別Hostによる品質差を禁止する
  `docs/architecture/FORGE-UNIVERSAL-QUALITY-INVARIANT.md` を追加した。
- Hardware/Model Profileを品質Tierではなく同一品質へ到達するExecution Resolver
  として再定義。低資源PCはReuse、CPU最適化、分割実行、許可済み別Hostで吸収し、
  Task/Visual/Safety/Privacy/Recovery/Accessibilityの共通Gateを下げない。
- `FORGE-SELF-CONTAINED-DISTRIBUTION.md`の`Low resource PC -> 小型モデル`を撤回。
  Local Model Policy、Machine-independent Policy、Agent Protocolも同じ原則へ統一。
- 0円戦略のLOC-10/11、Z2/Z9、Work Package、Recovery Matrix、Q081/Q083/Q144/
  Q208/Q230、Hard Gateを修正。最小限の道具は合意Scopeの段階化であり、品質縮小・
  無断意味削除・低品質な代替成功ではないと固定した。
- 音声/文字、Mobile/Tablet/Desktop/Web、動作するControl、正直な完了説明、
  現在状態からの再生成をCore UX Hard Gateへ追加した。
- `scripts/check_universal_quality_policy.py`とCI stepを追加し、危険な端末→小型Model
  直結や`Visible degraded state`の再導入を検出する。
- Constitution変更手続きに従って提案の正確な文面をCEOへ提示し、
  「いいよ、すべて承認」の明示承認後、Universal Quality Invariantを§13へ
  正式追記した。変更提案も`APPROVED BY CEO / APPLIED`へ更新した。
- TD92の製品判断は「release無言消失禁止、低品質な代替成功禁止」に確定。
  Runtime実装・実Render・Visual Evidenceは未実装のため、負債自体は継続する。
- Runtime/product codeの成功状態は変更していない。監査結果は
  `FORGE-POLICY-ALIGNMENT-AUDIT-20260902.md`に記録した。
...
実描画すると AppBar に「毎日の収入と支出を記録して残高を見たい」と
出ていた——**目的（文）を名前（名詞句）として使っていた**。

### `forge_ai/core/naming.py`（新設）
...
落とすもの: 句読点 / 願望・依頼の語尾（したい・ほしい・して・
できるように）/ 14文字超 / **英小文字の内部識別子**（`item`）。

**願望文から名詞句を削る処理は1つも足していない。削らずに落とす。**
「残高を見たい」→「残高を見」のような半端に壊れた名前を作らないため。

出所では弾かない。「買い物リストを作りたい」→「買い物リスト」は
短い名詞句なので**通す**。見るのは形であって出所ではない。

compile の**2経路とも**通した（`compiler.py` /
`ir/forge_language_compiler.py`）。名付けが1か所に集まる。
...
「子どもが朝の支度を…」は `child_growth` と判定され、「こどもの成長」
という名前で体重測定・身長測定が並んだ。「旅行の写真を…」は `travel`
と判定され、「旅行」という名前の持ち物リストになった。

これは名付けの失敗ではなく **Domain 判定の失敗**である。第2回までは
要求文がタイトルに出ていたので、画面が要求とずれていても**タイトルだけは
正しく見えていた**。それが誤判定を隠していた。隠れている不具合より、
見えている不具合の方がよい。

**(b) analytics / game / study が全 viewport でバイト単位一致した。**
第3回で悪化したのではない。元から同じだったものが、飾り（要求文の
タイトル）が取れて見えるようになっただけである（TD87）。

### 配線破壊試験: 6件すべて落ちた（置物なし）

M1 legacy compiler の配線 / M2 IR compiler の配線 / M3 `is_name_like`
を常に True / M4 内部識別子ガード / M5 `generic` も名前に使う /
M6 GENERIC の代わりに要求文を返す。

M4 だけは HTTP のテストでは落ちない（本番の Entity ラベルは日本語）。
`test_pipeline.py` の `run_cognitive_pipeline` E2E が落とす。
**正直に区別して書く。**

### 「作ったが本番から呼ばれない」を作らない

`backend/tests/test_generated_app_naming.py` は**本番の HTTP** を叩き、
撮影対象と**同じ8つの Need**で `app.title` / `screen.title` を見る。
絵とテストが別の入力を見ていると、絵で見つけた問題を固定できない。

### 自分の書いたテストが1件間違っていた

`assertNotIn(title, need)` と書いたら**正しい名前が落ちた**——
「旅行の写真を…」に対する「旅行」は要求文の部分文字列でありながら
良い名前である。名前が要求と語を共有するのは当たり前。行を消して
理由をテストへ書いた。

### 検証

```
backend  : 1744 passed, 16 skipped  (+5)
forge_ai :  537 passed              (+16)
flutter  : analyze No issues found / 514 passed
```
